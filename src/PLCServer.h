...
private:
	...
	void onReadCommand(const boost::json::value& data_frame) {
		std::string cmd;
		auto js_data_obj = data_frame.if_object();
		if(!js_data_obj) {
			SendErrorMsg("Parsing Error: Json data frame must be an object"); 
			return; 
		}
		auto js_cmd = js_data_obj->if_contains("Cmd");
		if(!js_cmd){
		...
		if      (cmd == "UNNOWN")       commandUnnown();
		else if (cmd == "PING")         commandPing();
		else if (cmd == "APP_START")    commandNotImplemented(data_frame, cmd);
		else if (cmd == "APP_STOP")     commandNotImplemented(data_frame, cmd);
		else if (cmd == "APP_RESUME")   commandNotImplemented(data_frame, cmd);
		else if (cmd == "APP_PAUSE")    commandNotImplemented(data_frame, cmd);
		else if (cmd == "MODULE_LIST")  commandNotImplemented(data_frame, cmd);
		else if (cmd == "TAG_LIST")     commandNotImplemented(data_frame, cmd);
		else if (cmd == "TAG_GET")      commandNotImplemented(data_frame, cmd);
		else if (cmd == "TAG_SET")      commandNotImplemented(data_frame, cmd);
		else if (cmd == "FILE_LIST")    commandNotImplemented(data_frame, cmd);
		else if (cmd == "FILE_SAVE")    commandNotImplemented(data_frame, cmd);
		else if (cmd == "FILE_READ")    commandNotImplemented(data_frame, cmd);
		else {
	...
	void commandNotImplemented(const boost::json::value& data_frame, std::string cmd){
	...
	void commandUnnown(){
		boost::json::value response = {{"Cmd","UNNOWN"}, {"Result","OK"}, {"Msg", "XD"}};
		sendJson(response);
	}


	void commandPing(){
		boost::json::value response = {{"Cmd","PING"}, {"Result","OK"}, {"Msg", "PONG"}};
		sendJson(response);
	}
